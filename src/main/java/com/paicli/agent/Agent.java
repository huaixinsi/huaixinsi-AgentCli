...
public class Agent {
    ...
    private static final int TRACE_PREVIEW_CHARS = 2_000;
        ...
        while (true) {
            ...
            try {
                ...
                if (response.hasToolCalls()) {
                    ...
                    renderTraceToolCalls(response.toolCalls(), iteration);

                    List<ToolExecutionResult> toolResults = executeToolCalls(response.toolCalls(), iteration);
                    renderTraceToolResults(toolResults, iteration);
    ...
    private boolean traceEnabled() {
        return truthy(System.getProperty("paicli.trace"))
                || truthy(System.getenv("PAICLI_TRACE"));
    }
    ...
    private void renderTraceToolCalls(List<LlmClient.ToolCall> toolCalls, int iteration) {
        if (!traceEnabled() || toolCalls == null || toolCalls.isEmpty()) {
            return;
        }
        PrintStream out = renderer().stream();
        for (LlmClient.ToolCall toolCall : toolCalls) {
            if (toolCall == null || toolCall.function() == null) {
                continue;
            }
            out.println(AnsiStyle.heading("执行工具[" + iteration + "]: " + toolCall.function().name()));
            out.println(AnsiStyle.subtle("参数: " + prettyJson(toolCall.function().arguments())));
        }
    }

    private void renderTraceToolResults(List<ToolExecutionResult> toolResults, int iteration) {
        if (!traceEnabled() || toolResults == null || toolResults.isEmpty()) {
            return;
        }
        PrintStream out = renderer().stream();
        for (ToolExecutionResult result : toolResults) {
            ...
            String elapsed = result.elapsedMillis() > 0 ? " (" + result.elapsedMillis() + "ms)" : "";
            out.println(AnsiStyle.subtle("结果[" + iteration + "]: " + result.name() + elapsed));
            out.println(preview(result.result(), TRACE_PREVIEW_CHARS));
        }
    }
