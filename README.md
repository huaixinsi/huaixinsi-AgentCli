...
当前进度：已完成第 16.1 期 inline 流式 TUI 形态修正、第 17 期 `LSP 诊断注入` MVP、第 18 期 `Git Side-History 快照与回滚` MVP、第 19 期 `Prompt 分层架构` MVP、第 20 期 `异步后台任务 + Runtime API` MVP、第 21 期 `图片复制粘贴输入` MVP、第 23 期 `Agent 自动路由` MVP。
...
- 普通输入先由 `AgentRouter` 自动选择 ReAct / Plan-and-Execute / Multi-Agent；`/plan <任务>` 和 `/team <任务>` 仍可强制指定路径；`/cancel` 可取消运行中任务
...
### 第二十三期：Agent 自动路由（MVP）

- 新增 `AgentRouter`：普通输入先做确定性打分，再选择 ReAct、Plan-and-Execute 或 Multi-Agent
- 显式 `/plan`、`/team` 最高优先级，不会被自动路由覆盖
...
普通输入默认由 `AgentRouter` 自动选择 ReAct / Plan-and-Execute / Multi-Agent。需要强制进入 `Plan-and-Execute` 时使用 `/plan`：
