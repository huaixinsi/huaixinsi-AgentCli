...
- `ROLLBACK`：第三期开始恢复当前 task 的 `pre-task` checkpoint，只撤销失败 task 的改动；如果 checkpoint 不可用，会返回清晰的回滚失败说明。
...
- 回滚依赖 Side-Git task checkpoint；没有可用 checkpoint 时不会退回自动 turn 级回滚或手写文件恢复逻辑。
- 分类器使用本地规则，不额外调用 LLM。
